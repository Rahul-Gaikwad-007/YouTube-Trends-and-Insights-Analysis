...
def start_analysis():
    ...
    with st.spinner('Scraping videos... This may take a moment.'):
        ...
        # Debugging: Log the response from trigger_scraping_channels
        ...
        st.write("Debug: channel_snapshot_id response:", channel_snapshot_id)
        
        # Check if 'snapshot_id' exists
        if 'snapshot_id' not in channel_snapshot_id:
            status_container.error("Error: 'snapshot_id' not found in the response.")
            return
        ...
        if status['status'] == "ready":
            ...
            for i in tqdm(range(len(channel_scrapped_output[0]))):
                if 'shortcode' not in channel_scrapped_output[0][i]:
                    st.warning(f"Video at index {i} is missing a shortcode.")
                    continue

                youtube_video_id = channel_scrapped_output[0][i]['shortcode']
                file = os.path.join(transcripts_dir, f"{youtube_video_id}.txt")
                st.session_state.all_files.append(file)

                with open(file, "w") as f:
                    if 'formatted_transcript' in channel_scrapped_output[0][i] and channel_scrapped_output[0][i]['formatted_transcript']:
